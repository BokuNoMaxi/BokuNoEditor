...
function setInformationen($Info){
    ...
    return '{\info{\title '.$Info['Titel'].'}{\author '.$Info['Author'].'}{\company '.$Info['Organisation'].'}{\creatim\yr'.$Info['Datum']->format('Y').'\mo'.$Info['Datum']->format('m').'\dy'.$Info['Datum']->format('d').'\hr'.$Info['Datum']->format('H').'\min'.$Info['Datum']->format('i').'}{\doccomm '.$Info['Kommentar'].'}}';
}
...
function makeFormatierungHTML2RTFParagraph($pArr){
    ...
    foreach ($pArr as $p){//arbeite das Array Paragraph für Paragraph ab
        ...
        $RTFParagraph= ((strpos($RTFContent['RTF'], '\trowd')!==false)?$RTFPraefix.$RTFContent['RTF']:makeParagraph($RTFPraefix.$RTFContent['RTF']));
    ...
    if($HTMLline!='<br>'){
        ...
        if(strpos($HTMLline ,'<tr')!==false){
            foreach(explode('<tr',$HTMLline) as $R){
                if(strpos($R,'</table>')){
                    $tableEnd=strpos($R,'</table>');
                    if(strpos($R,'<br>',$tableEnd)){
                        $R= str_replace('<br>', '\pard\par',$R,$tableEnd);
                    }
                    $R= str_replace('</table>', '', $R);
                }
                ...
                if(strstr($R,'\cell')){
                    ...
                    foreach($TDStylings as $index=>$ST){
                        ...
                        $TableUnformatedContent.=SonderzeichenWandler('HTML',$TableUnformatedContentActive);
                    }
        ...
        $HTMLline=str_replace("<br>", br, $HTMLline);//ersetze alle BRs durch Lines 
    }else{
